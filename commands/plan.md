...
## Шаг 0.5: Параллельный ресёрч (только Режим A — новый проект)

Если это новый проект и владелец описал стек/домен — запусти **параллельных ресёрчеров**
через Task tool (subagent_type=Explore) для сбора контекста:

```
Запустить параллельно (3 агента):

1. Stack Research — какие библиотеки/фреймворки лучше подходят,
   актуальные версии, известные проблемы совместимости

2. Architecture Research — типичные паттерны архитектуры
   для этого типа приложения, data flow, структура проекта

3. Pitfalls Research — распространённые ошибки, подводные камни,
   deprecated API, проблемы производительности
```

Каждый агент помечает findings по confidence:
- **HIGH** — проверено через Context7 / официальную документацию
- **MEDIUM** — несколько источников согласны
- **LOW** — один источник, требует валидации

**Покажи владельцу ключевые findings** перед обсуждением целей.
Это помогает формировать реалистичные цели с учётом технических ограничений.

**Если владелец не хочет ресёрч** или стек уже хорошо знаком — пропусти этот шаг.
