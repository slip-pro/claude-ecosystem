...
async function fetchBoardRest(boardId: string) {
...
async function fetchCardRest(cardId: string) {
...
async function fetchBoardPrisma(boardId: string) {
...
async function fetchCardPrisma(cardId: string) {
  const db = await getPrisma();
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function formatBoardText(board: any): string {
  const now = new Date();
  let totalCards = 0;
  let blockedCount = 0;
  let overdueCount = 0;
  ...
  for (const column of board.columns) {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      lines.push(
        `   Assignees: ${getAssigneeNames(assignees)}`
      );
  ...
  lines.push(
    `Summary: ${totalCards} active cards across ` +
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function formatCardText(card: any): string {
  const lines: string[] = [];
  ...
  if (card.isBlocked) {
    ...
    if (card.blockedByCard) {
      lines.push(
        `   Blocked by: ${card.blockedByCard.title}` +
  ...
  if (card.comments?.length > 0) {
    ...
    for (const comment of [...card.comments].reverse()) {
      ...
      lines.push('');
      lines.push(
        `**${author}** (${formatDate(comment.createdAt)}):`
      );
...
server.tool(
  'get_board_tasks',
  'Get current board state: columns, cards, priorities,' +
    ' assignees, due dates. Use at the start of planning.',
...
server.tool(
  'get_card_details',
  'Get detailed card info: description, checklists,' +
    ' comments, activity. Use before starting task work.',
  { cardId: z.string() },
  async ({ cardId }) => {
    try {
...
function requireRestApi(): string | null {
  if (!useRestApi) {
    return (
      'Write operations require REST API.' +
...
server.tool(
  'move_card',
  'Move card to a different column (change status).' +
  ...
  async ({ cardId, targetColumnId }) => {
    ...
    try {
      const card = await restPost(
        `/api/v1/boards/cards/${cardId}/move`,
        { targetColumnId, newOrder: 0 }
      );
      const columnName =
        card.column?.name || targetColumnId;
      return {
        content: [
          {
            type: 'text' as const,
...
server.tool(
  ...
  async ({ cardId, content }) => {
    ...
    try {
      const comment = await restPost(
        `/api/v1/boards/cards/${cardId}/comments`,
        { content }
      );
...
server.tool(
  'update_card',
  'Update card: title, description, priority,' +
    ' due date, color.',
  {
    cardId: z.string().describe('Card ID'),
